...
public class DContainer extends DComponent implements DParent {
	...
	/**
	 * method adds a new DElement to the container if the container already
	 * contains this element, the method simply returns the method checks
	 * whether the DElement is an ancestor of the container itself. In that case
	 * it throws an IllegalArgumentException.
	 * 
	 * @param e
	 *            the new DElement to add
	 */
	...
	/**
	 * method adds a new DElement to the container by a certain key name if the
	 * container already contains this element, the method simply returns the
	 * method checks whether the DElement is an ancestor of the container
	 * itself. In that case it throws an IllegalArgumentException.
	 * 
	 * @param key
	 *            the name of the DElement in the container
	 * @param e
	 *            the new DElement to add
	 */
			...
			while (me != null) {
				if (he == me)
					throw new IllegalArgumentException(
							"Adding DContainer's parent to itself");
	...
	/**
	 * method removes a certain DElement from the container an returns whether
	 * it contained it before
	 * 
	 * @param e
	 *            the DElement to remove
	...
	/**
	 * method returns the first DElement of the contaioner belonging to the
	 * given key
	 * 
	 * @param key
	 *            the name of the DElement in the container
	...
	public DElement getDElement(String key) {
		...
		for (int i = 0; index == -1 && i < keys.size(); i++)
			if (((String) keys.get(i)).equals(key))
				index = i;
		return (index < keys.size()) ? (DElement) elements.get(index)
				: (DElement) null;
	}
	...
	public void paint(DMeasures m) {
		...
		for (int i = 0; i < elements.size(); i++) {
			...
			if (e.isVisible()
					&& !m.getDRectangle().getIntersection(e.getRectangle())
							.isEmpty()) {
	...
	/**
	 * method adds the given border to the current border of the container that
	 * means, that if necessary it will enlarge it and tells it to its parent
	 * 
	 * @param b
	 *            the border to add
	 */
	...
	/**
	 * restores the container, that means that the rectangle is completely new
	 * calculated this method is used after removing one of the elements
	 */
	boolean restore() {
