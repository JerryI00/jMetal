...
/**
 * This serves as activation state of one item in an array of Tags identified by
 * integer IDs. String names should be unique as well as integer IDs.
 */
...
public class SelectedTag implements java.io.Serializable {
	...
	/**
	 * Constructor from a String array, creates a Tag array where the IDs
	 * correspond to array index. Standard selection is 0.
	 * 
	 * @param strings
	 */
	...
	/**
	 * Constructor with a given Tag array. The IDs should correspond to the
	 * array index.
	 * 
	 * @param selID
	 * @param tags
	...
	public SelectedTag(int selID, Tag[] tags) {
		...
		for (int i = 0; i < tags.length; i++) {
			if (i != tags[i].getID()) {
				System.err
						.println("warning, SelectedTag with inconsistent ID, this may cause problems");
			}
	...
	/**
	 * Set the selected tag by String tag name. If the given name doesnt exist,
	 * nothing will change and an error message will be printed to System.err.
	 * This should of course be avoided.
	 * 
	 * @param str
	 *            The new selected tag name
	 */
	public void setSelectedTag(String str) {
		...
		System.err
				.println("Warning, trying to select unknown string (SelectedTag::setSelectedTag(String)");
	}

	/**
	 * Sets the local selection to the same ID currently selected by the given
	 * instance. If the ID is not valid, nothing is changed.
	 * 
	 * @param o
	 *            another SelectedTag instance, preferably with the same tags
	 */
	...
	/**
	 * Returns true if the given String is equivalent to the currently selected
	 * Tags string component, else false.
	 * 
	 * @param str
	 *            String to compare to
	 * @return true if the given string is equivalent to the currently selected
	...
	/**
	 * Check if the given object has the same string tags as this one and return
