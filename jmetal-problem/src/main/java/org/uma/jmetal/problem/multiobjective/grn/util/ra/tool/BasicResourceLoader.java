...
///////////////////////////////////////////////////////////////////////////////
package org.uma.jmetal.problem.multiobjective.grn.util.ra.tool;
...
public class BasicResourceLoader implements ResourceLoader {
	 ...
	 */
	private static DummyCategory logger = DummyCategory
			.getInstance(BasicResourceLoader.class.getName());
	 ...
	 *            Description of the Parameter
	 * @param ignorePrefix
	 *            array of prefixes which mark a line to be ignored
	 * @param lOffset
	 *            offset of the first line to read
	 * @param lCnt
	 *            number of lines to read, if <= 0, all lines are read
	 * @return Description of the Return Value
	 */
	public static List<String> readLines(String resourceFile,
		...
		byte[] bytes = BasicResourceLoader.instance()
				.getBytesFromResourceLocation(resourceFile);
		...
		try {
			while ((line = lnr.readLine()) != null) {
				...
				// if ((ignorePrefix == null) || (strStartsWithPrefix(line,
				// ignorePrefix) < 0)) {
				// // if there are no prefixes given or none of them fits, add
				// the line
				// lineData.add(line);
				// }
				// }
	...
	/**
	 * Parse columns of a data array containing double data. Columns may be
	 * selected by giving their indices in an int array. If selectedCols is
	 * null, all columns are selected. All selected columns are expected to
	 * contain double data and to be of same length. If rawData is null, null is
	 * returned.
	 * 
	 * @param rawData
	 *            Strings containing an array with double data columns
	 * @param colSplit
	 *            String regexp for the splitting of a line
	 * @param selectedCols
	 *            indices of the columns to retrieve, null for all.
	 * @see java.util.regex.Pattern
	 * @return
	 */
	public static double[][] parseDoubleArray(ArrayList<String> rawData,
			String colSplit, int[] selectedCols) {
		String[] entries;
		double dat[][] = null;
		if (rawData != null) {
			try {
				for (int i = 0; i < rawData.size(); i++) {
					...
					if (i == 0) { // at the first pass
						dat = new double[rawData.size()][(selectedCols == null) ? entries.length
								: selectedCols.length];
					}
	...
	/**
	 * Walk through a 2-d-array and retrieve the first bunch of lines for which
	 * the given column data lies within start and end limits, both inclusively.
	 ...
	 *            column to inspect
	 * @param start
	 *            first value to start retrieving from
	 * @param end
	 *            last value to retrieve
	 * @return
	 */
	public static double[][] getLinesByCol(double[][] data, int col,
	...
	/**
	 * Load double data from a text file. An ignore list of prefixes may be
	 * specified. The start line and number of lines to read may be specified,
	 * if lCnt is -1, as many lines as possible are read. The cols array may
	 * contain an integer list of columns to be read. If null, as many columns
	 * as possible are read. The data file is expected to be uniform, meaning
	 * that all lines which are not ignored, contain double data values in all
	 * columns.
	 * 
	 * @param fname
	 *            file name to read
	 * @param ignorePrefix
	 *            lines starting with any of these Strings will be ignored
	 * @param colSplit
	 *            String regexp for the splitting of a line
	 * @param lOffset
	 *            start at a certain line (0 for top)
	 * @param lCnt
	 *            read as many lines, -1 or 0 for all (from offset). Ignored
	 *            lines are not counted!
	 * @param selectedCols
	 *            indices of the columns to retrieve, null for all.
	 * @return
	 */
	public static double[][] loadDoubleData(String fname,
			String[] ignorePrefix, String colSplit, int lOffset, int lCnt,
			int[] selectedCols) {
		return parseDoubleArray((ArrayList<String>) readLines(fname,
				ignorePrefix, lOffset, lCnt), colSplit, selectedCols);
	}

	/**
	 * Fill a line of an array with double values parsed from a String array. A
	 * subset of Columns may be selected by giving their indeces in an integer
	 * array cols. If cols is null, all are converted.
	 * 
	 * @param dest
	 * @param lineCnt
	 * @param entries
	 * @param cols
	 */
	public static void fillLine(double[][] dest, int lineCnt, String[] entries,
			int[] cols) {
		if (((cols == null) && (dest[lineCnt].length != entries.length))
				|| (cols != null && (dest[lineCnt].length != cols.length))) {
			System.err
					.println("error, array dimensions dont match! (BasicResourceLoader)");
		}
		if (cols == null) {
			for (int i = 0; i < entries.length; i++) {
				try {
					...
					System.err.println("Invalid Double format in line "
							+ lineCnt + ", data was " + entries[i]);
				}
			}
		} else {
			for (int i = 0; i < cols.length; i++) {
				try {
					...
					System.err.println("Invalid Double format in line "
							+ lineCnt + ", data was " + entries[cols[i]]);
				}
			}
		}
	}
	...
	/**
	 * Test a string for prefixes. If a prefix matches, return its index, else
	 * return -1.
	 * 
	 * @param str
	 * @param pref
	 ...
	 * 
	 * @param urlToZipArchive
	 *            Description of the Parameter
	 * @param internalArchivePath
	 *            Description of the Parameter
		...
		try {
			...
			while (e.hasMoreElements()) {
				...
				if (ze.getName().equals(internalArchivePath)) {
					...
					if (ze.getSize() > 65536) {
						System.out
								.println("Resource files should be smaller than 65536 bytes...");
					}
	...
	public static Properties readProperties(String resourceName)
		...
		if (propFile.exists()) {
			try {
				...
				System.out.println("Problem reading user properties: "
						+ propFile);
			}
		}
		return userProps;
