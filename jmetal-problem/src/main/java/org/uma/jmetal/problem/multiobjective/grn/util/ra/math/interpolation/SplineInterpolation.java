...
public class SplineInterpolation {
	...
	/**
	 * Initializes this class and calculates the second derivative of the
	 * spline.
	 * 
	...
	public void setAbstractDataSet(AbstractDataSet abstractDataSet)
		...
		for (int i = 0; i < n - 1; i++) {
			...
			yArray[i] = y[n - i - 1];
			if (x[i] < x[i + 1]) {
				// if(descending)throw new
				// InterpolationException("The x values must be"+
				// " in continous ascending/descending order.");
				ascending = true;
			} else {
				// if(ascending)throw new
				// InterpolationException("The x values must be"+
				// " in continous ascending/descending order.");
				descending = true;
			}
		...
		this.secondDerivative = spline(xArray, yArray, (yArray[1] - yArray[0])
				/ (xArray[1] - xArray[0]), (yArray[n - 1] - yArray[n - 2])
				/ (xArray[1] - xArray[n - 2]));
	}
	...
	public double getDerivative(double x) throws InterpolationException {
		return splineInterpolatedDerivative(xArray, yArray, secondDerivative, x);
	}
	 ...
	 * x<sub>0</sub>&lt;x<sub>1</sub>&lt;x<sub>2</sub>&lt;... . This algorithm
	 * was taken from:<br>
	 * <a href="http://www.ulib.org/webRoot/Books/Numerical_Recipes/"
	 * target="_top"> William H. Press, Saul A. Teukolosky, William T.
	 * Vetterling, Brian P. Flannery,
	...
	public double[] spline(double[] x, double[] y, double yp0, double ypn)
			throws InterpolationException {
		if (x[0] > x[1])
			throw new InterpolationException("The x values must be"
					+ " in ascending order.");
		int n = x.length;
	...
	/**
	 * Calculates the second derivative of the data. This algorithm was taken
	 * from:<br>
	 * <a href="http://www.ulib.org/webRoot/Books/Numerical_Recipes/"
	 * target="_top"> William H. Press, Saul A. Teukolosky, William T.
	 * Vetterling, Brian P. Flannery,
	 ...
	 * 113-116.</a><br>
	 */
	public double splineInterpolation(double[] xa, double[] ya, double[] y2a,
	 ...
	 * 113-116.</a><br>
	 */
	public double splineInterpolatedDerivative(double[] xa, double[] ya,
			double[] y2a, double x) throws InterpolationException {
		int n = xa.length;
		if (n != ya.length || n != y2a.length) {
			throw new InterpolationException("Arrays have different lengths.");
		}
		...
		a = (xa[khi] - x) / h;
		b = (x - xa[klo]) / h;
		dydx = (ya[khi] - ya[klo]) / h - ((3 * (a * a) - 1) * h * y2a[klo])
				/ 6.0 + ((3 * (b * b) - 1) * h * y2a[khi]) / 6.0;
		return dydx;
	}
}
