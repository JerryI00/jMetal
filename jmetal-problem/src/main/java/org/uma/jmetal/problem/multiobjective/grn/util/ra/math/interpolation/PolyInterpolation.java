...
package org.uma.jmetal.problem.multiobjective.grn.util.ra.math.interpolation;
...
public class PolyInterpolation {
	 ...
	 *            the <code>AbstractDataSet</code>
	 * @param sloppy
	 *            if <code>true</code> Neville's algorithm which is used in the
	 *            <code>polynomialInterpolation</code>-routines does only print
	 *            a warning message on the screen and does not throw an
	 ...
	 * <a href="http://www.ulib.org/webRoot/Books/Numerical_Recipes/"
	 * target="_top"> William H. Press, Saul A. Teukolosky, William T.
	 * Vetterling, Brian P. Flannery,
	 * "Numerical Recipes in C - The Art of Scientific Computing", Second
	 * Edition, Cambridge University Press, ISBN 0-521-43108-5, chapter 5, pages
	 * 173-176.</a><br>
	 * The Neville's algorithm which is used in the
	 * <code>polynomialInterpolation</code>- routines returns also the error of
	 * this interpolated point.
	 * 
	 * @param x
	 *            the x value
	 ...
	 * @param x
	 *            the x value
	 * @param ndDerivateNumber
	 *            the number of the calculated derivatives
	...
	public double[] getYandDerivatives(double x, int ndDerivateNumber)
		...
		if (ndDerivateNumber < 0)
			throw new InterpolationException(
					"Negative derivative numbers make no sense.");
	 ...
	 * Vetterling, Brian P. Flannery,
	 * "Numerical Recipes in C - The Art of Scientific Computing", Second
	 * Edition, Cambridge University Press, ISBN 0-521-43108-5, chapter 3, pages
	 * 108-122.</a><br>
	 * 
	 * @param x
	 *            the x value
	...
	public PolynomialInterpolationResult polynomialInterpolation(double x)
		...
		if (abstractDataSet == null)
			throw new InterpolationException("No data."
					+ " The AbstractDataSet was not defined.");
		return polynomialInterpolation(abstractDataSet.getXData(),
				abstractDataSet.getYData(), x);
	}
	...
	public PolynomialInterpolationResult polynomialInterpolation(
			AbstractDataSet abstractDataSet, double x)
			throws InterpolationException {
		if (abstractDataSet == null)
			throw new InterpolationException("No data."
					+ " The AbstractDataSet was not defined.");
	 ...
	 * @see #calculatePolynomialCoefficients(double[], double[])
	 */
	public PolynomialInterpolationResult polynomialInterpolation(double[] xa,
			double[] ya, double x) throws InterpolationException {
		if (xa == null || ya == null)
			throw new InterpolationException("No data.");
		int i, m, ns = 1;
		...
		PolynomialInterpolationResult result = new PolynomialInterpolationResult();
		...
		for (m = 1; m < xa.length; m++) {
			for (i = 1; i <= xa.length - m; i++) {
				...
				if ((den = ho - hp) == 0.0) {
					if (sloppy) {
						System.out
								.println("Two identical x values. The values must be distinct.");
						den = 1.0;
					} else
						throw new InterpolationException(
								"Two identical x values.");
				}
				den = w / den;
				d[i] = hp * den;
				c[i] = ho * den;
			}
			result.y += (result.yError = (2 * ns < (xa.length - m) ? c[ns + 1]
					: d[ns--]));
		}
		return result;
	}
	...
	/**
	 * Calculates the coefficients of a polynom of the grade <code>N-1</code>.
	 * This interpolation algorithm was taken from:<br>
	 * <a href="http://www.ulib.org/webRoot/Books/Numerical_Recipes/"
	 * target="_top"> William H. Press, Saul A. Teukolosky, William T.
	 * Vetterling, Brian P. Flannery,
	 * "Numerical Recipes in C - The Art of Scientific Computing", Second
	...
	public double[] calculatePolynomialCoefficients()
		...
		if (abstractDataSet == null)
			throw new InterpolationException("No data."
					+ " The AbstractDataSet was not defined.");
		return calculatePolynomialCoefficients(abstractDataSet.getXData(),
