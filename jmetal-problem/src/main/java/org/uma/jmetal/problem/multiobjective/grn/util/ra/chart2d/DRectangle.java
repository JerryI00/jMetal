...
public class DRectangle extends DComponent {
	...
	public DRectangle(double x, double y, double width, double height) {
		...
		if (width < 0)
			throw new IllegalArgumentException(
					"Width of a DRectangle has to be >= 0");
		this.width = width;
		if (height < 0)
			throw new IllegalArgumentException(
					"Height of a DRectangle has to be >= 0");
		this.height = height;
		status = PART;
	}
	...
	public void paint(DMeasures m) {
		...
		Point p1 = m.getPoint(rect.x, rect.y), p2 = m.getPoint(rect.x
	...
	public boolean contains(DRectangle rect) {
		...
		if (status == EMPTY || rect.isAll())
			return false;
		if (!contains(new DPoint(rect.x, rect.y)))
			return false;
		if (!contains(new DPoint(rect.x + rect.width, rect.y + rect.height)))
			return false;
		return true;
	}
	...
	/**
	 * method inserts the given rectangle to this instance of it and returns
	 * true when the size changed
	 * 
	 * @param rect
	 *            the rectangle to inserts
	...
	public boolean insert(DRectangle rect) {
		...
		changed = insert(new DPoint(rect.x + rect.width, rect.y + rect.height)) ? true
				: changed;
		return changed;
	}
