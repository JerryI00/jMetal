...
package org.uma.jmetal.problem.multiobjective.grn.util.ra.print;
...
public class PagePrinter {
	...
	public int print() {
		...
		double w = (int) pf.getImageableWidth(), h = (int) pf
				.getImageableHeight();
		...
		if (old.width > w || old.height > h) {
			...
			JCheckBox cbFitIn = new JCheckBox("fitted-in", rec_fit_in), cbTurn = new JCheckBox(
					"turned", rec_turn);
			...
			if (choice == JOptionPane.CANCEL_OPTION
				...
				if (cbTurn.isSelected()) {
					...
					if (cbFitIn.isSelected()) {
						double m = Math.min(h / (double) old.width, w
								/ (double) old.height);
						img = (BufferedImage) c.createImage(
								(int) (old.height * m), (int) (old.width * m));
						Graphics2D g2 = img.createGraphics();
						g2.rotate(Math.toRadians(90));
						g2.translate(0, -old.height * m);
						c
								.setSize((int) (old.width * m),
										(int) (old.height * m));
						c.paint(g2);
						c.setSize(old);
					} else {
						img = (BufferedImage) c.createImage(old.height,
								old.width);
						Graphics2D g2 = img.createGraphics();
						g2.rotate(Math.toRadians(90));
					...
					if (cbFitIn.isSelected()) {
						double m = Math.min(w / (double) old.width, h
								/ (double) old.height);
						c
								.setSize((int) (old.width * m),
										(int) (old.height * m));
