...
package org.uma.jmetal.problem.multiobjective.grn.util.ra.chart2d;
...
/**
 * ScaledBorder puts an border around Components ( especially around
 * DrawingAreas ) with scaled and labeled axes
 */
public class ScaledBorder implements Border {
	 ...
	 */
	int x_values2arrow = 10, y_values2arrow = 10;
	 ...
	 */
	public int x_label2border = 6, y_label2border = 6;

	/**
	 * the size of the source rectangle that means before the values are mdified
	 * by scale functions
	 */
	DRectangle src_rect;
	...
	/**
	 * scale functions if, for example, an logarithmic function is needed
	 * instead of a linear.
	 */
	...
	private boolean do_refresh, auto_scale_x = true, auto_scale_y = true;
	...
	/**
	 * constructor creates a default ScaledBorder inside of a lowered
	 * BevelBorder
	 */
	public ScaledBorder() {
		this(BorderFactory.createBevelBorder(BevelBorder.LOWERED, Color.white,
   ...
   * surrounded by the specified <code>Border<code/>
	...
	/**
	 * method sets the differences between source x-values of the displayed
	 * values
	 * 
	 * @see setAutoScaleX(). If scale functions are used there might be a
	 *      difference between the shown values and the source values
	 */
	...
	/**
	 * method sets the differences between source y-values of the displayed
	 * values
	 * 
	 * @see setAutoScaleY(). If scale functions are used there might be a
	 *      difference between the shown values and the source values
	 */
	...
	public void paintBorder(Component c, Graphics g, int x, int y, int width,
		...
		Insets outer_insets = new Insets(0, 0, 0, 0);// insets of the outer
														// border
		...
		Dimension d = c.getSize(), cd = new Dimension(d.width
				- inner_insets.left - inner_insets.right, d.height
				- inner_insets.top - inner_insets.bottom);
		...
		g.drawLine(inner_insets.left, inner_insets.top, inner_insets.left,
		...
		if (show_arrows) {
			g.drawLine(inner_insets.left, inner_insets.top, inner_insets.left,
					inner_insets.top - y_values2arrow);
			g.drawLine(inner_insets.left - marker_length, inner_insets.top
					- y_values2arrow, inner_insets.left, inner_insets.top
					- y_values2arrow - arrow_length);
			g.drawLine(inner_insets.left + marker_length, inner_insets.top
					- y_values2arrow, inner_insets.left, inner_insets.top
					- y_values2arrow - arrow_length);
			g.drawLine(inner_insets.left - marker_length, inner_insets.top
					- y_values2arrow, inner_insets.left + marker_length,
					inner_insets.top - y_values2arrow);

			g.drawLine(inner_insets.left + cd.width, inner_insets.top
					+ cd.height, inner_insets.left + cd.width + x_values2arrow,
					inner_insets.top + cd.height);
			g.drawLine(inner_insets.left + cd.width + x_values2arrow,
					...
					inner_insets.left + cd.width + x_values2arrow
							+ arrow_length, inner_insets.top + cd.height);
			g.drawLine(inner_insets.left + cd.width + x_values2arrow,
					inner_insets.top + cd.height + marker_length,
					inner_insets.left + cd.width + x_values2arrow
							+ arrow_length, inner_insets.top + cd.height);
		...
		if (y_label != null) {
			Dimension yld = new Dimension(fm.getAscent() + fm.getDescent(), fm
					.stringWidth(y_label));
			AffineTransform T = new AffineTransform(0, -1, 1, 0, 0, 0);
			Font old = g.getFont(), f = old.deriveFont(T);
			g.setFont(f);
			g.drawString(y_label, y_label2border + fm.getAscent(),
					inner_insets.top + (cd.height + yld.height) / 2);
			g.setFont(old);
		}

		if (x_label != null)
			g.drawString(x_label, inner_insets.left
					+ (cd.width - fm.stringWidth(x_label)) / 2, d.height
					- outer_insets.bottom - x_label2border - fm.getDescent());

		if (src_rect.x == 0 && src_rect.y == 0) {
			int v2m = fm.stringWidth("0") / y_values2marker;
			g.drawString("0", inner_insets.left - fm.stringWidth("0") - v2m
					- marker_length, inner_insets.top + cd.height + fontAsc);
			g.drawLine(inner_insets.left, inner_insets.top + cd.height
					+ fm.getAscent(), inner_insets.left, inner_insets.top
					+ cd.height);
			g.drawLine(inner_insets.left, inner_insets.top + cd.height,
					inner_insets.left - fm.stringWidth("0") - v2m
							- marker_length, inner_insets.top + cd.height);
		}
	...
	private void drawYValues(Graphics g, Insets insets, Dimension cd) {
		...
		FontMetrics fm = g.getFontMetrics();
		int n, fontAsc = fm.getAscent(), v2m = fm.stringWidth("0")
				/ y_values2marker;
		...
		for (; (v = n * src_dY) <= src_rect.y + src_rect.height; n++) {
			...
			try {
				v = format_y.parse(text).doubleValue();
			} catch (java.text.ParseException ex) {
			}
			Point p = m.getPoint(minx, v);
			if (p != null) {
				g.drawString(text, insets.left - fm.stringWidth(text) - v2m
						- marker_length, p.y + fontAsc / 2);
	...
	public double getSrcdY(FontMetrics fm, Dimension cd) {
		...
		double minsrc_dY = 2 * src_rect.height / (double) max; // die 2 einfach
																// mal so
																// eingesetzt
																// <--------------------------
	...
	private void drawXValues(Graphics g, Insets insets, Dimension cd) {
		...
		int n, labelX, xnull = insets.left + (int) (-src_rect.x * mx);
	...
	public double getSrcdX(FontMetrics fm, Dimension cd) {
		...
		if ((!do_refresh && src_dX != -1) || !auto_scale_x)
			return src_dX;
		int digit_width = fm.stringWidth("0"), max = cd.width
				/ (digit_width * (x_value2value + 1));
		...
		while (!ok) {
			...
			while (suits && n * src_dX <= src_rect.x + src_rect.width) {
				...
				String text = format_x.format(v);
				int strW = fm.stringWidth(text);
				labelX = (int) (((n * src_dX - src_rect.x) / src_rect.width) * cd.width)
						- strW / 2;
	...
	/**
	 * method returns to a certain minimal value the next higher value which can
	 * be displayed, which looks a bit nicer it returns values like ... 0.05,
	 * 0.1, 0.2, 0.5, 1, 2, 5, 10, ...
	 * 
	 * @param the
	 *            double value next to which the displayable value should be
	 *            found
	...
	public Insets getBorderInsets(Component c) {
		...
		FontMetrics fm = g.getFontMetrics();
		// int fontAsc = fm.getAscent();
		int fontHeight = fm.getHeight(), digit_width = fm.stringWidth("0");
		...
		insets.left += 1 + y_label2border + maxWidth + digit_width
				/ y_values2marker + marker_length;
