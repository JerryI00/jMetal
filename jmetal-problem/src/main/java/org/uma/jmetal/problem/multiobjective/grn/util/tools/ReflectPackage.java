package org.uma.jmetal.problem.multiobjective.grn.util.tools;

import org.uma.jmetal.problem.multiobjective.grn.util.ra.tool.BasicResourceLoader;
...
/**
 * Allow for java to list Classes that exist in one package and can be
 * instantiated from the classpath, either directly or through a jar on the
 * classpath. So far, jars which are located <i>within</i> another jar will not
 * be searched.
 * 
 * @author mkron
 * 
...
public class ReflectPackage {
	...
	// static HashMap<String, ArrayList<String>> pathMap = new HashMap<String,
	// ArrayList<String>>();
	 ...
	 */
	public static int getClassesFromFilesFltr(HashSet<Class> set, String path,
			String pckgname, boolean includeSubs, Class reqSuperCls) {
		try {
	...
	// public static ArrayList<Class> getClassesFromDir(File directory, String
	// pckgname, boolean includeSubs) {
	// return getClassesFromDirFltr(directory, pckgname, includeSubs, null);
	// }

	public static int getClassesFromDirFltr(HashSet<Class> set, File directory,
			String pckgname, boolean includeSubs, Class<?> reqSuperCls) {
		int cntAdded = 0;
		if (directory.exists()) {
			// Get the list of the files contained in the package
	...
	/**
	 * If valid classpath entries are stored but you want to reset them, use
	 * this method. The classpath will then be rescanned on the next request.
	 */
	...
	public static ArrayList<Class> filterAssignableClasses(
			ArrayList<Class> classes, Class<?> reqSuperCls) {
	 ...
	 * @return
	 */
	public static int getClassesFromJarFltr(HashSet<Class> set, String jarName,
			String packageName, boolean includeSubs, Class<?> reqSuperCls) {
	...
	public static Class[] getAllClassesInPackage(String pckg,
		...
		return getClassesInPackageFltr(new HashSet<Class>(), pckg, includeSubs,
				bSort, null);
	}
	...
	public static Class[] getClassesInPackageFltr(HashSet<Class> set,
			String pckg, boolean includeSubs, boolean bSort, Class reqSuperCls) {
	...
	/**
	 * Request a valid absolute resource path from a relative resource path by
	 * searching the classpath entries. Returns null if the resource is not
	 * found.
	 * 
	 * @param res
	 */
	public static String getResourcePathFromCP(String res) {
		...
		if (TRACE)
			System.out.println(res + ((url == null) ? " not" : " was")
		...
		for (int i = 0; i < cpEntries.length; i++) {
			if (!cpEntries[i].endsWith(".jar")) { // its a fs directory
													// (hopefully)
				if (TRACE)
					System.out.println("reading element " + cpEntries[i]);
				f = new File(cpEntries[i] + fNameSep);
				if (TRACE)
					System.out.println(res + ((!f.exists()) ? " not" : " was")
							+ " found in " + cpEntries[i]);
				if (f.exists())
	...
	public static InputStream getResourceStreamFromCP(String res) {
		...
		InputStream in = BasicResourceLoader.instance()
				.getStreamFromResourceLocation(res);
		if (TRACE)
			System.out.println(res + ((in == null) ? " not" : " was")
					+ " found by classloader.");
		if (in != null)
			return in;
	...
	public static void main(String[] args) {
		...
		// byte[] b =
		// rld.getBytesFromResourceLocation("resources/images/Sub24.gif");
		// System.out.println((b == null) ? "null" : b.toString());
		// b =
		// rld.getBytesFromResourceLocation("src/eva2/client/EvAClient.java");
		// System.out.println((b == null) ? "null" : b.toString());
		...
		// System.out.println(pathElements[i].endsWith(".jar") ? " is a jar" :
		// " is a path");
		// if (pathElements[i].endsWith(".jar")) {
		// ArrayList al = getClassesFromJarFltr(pathElements[i],
		// "eva2.server.model", false, null);
		// for (int k=0; k<al.size(); k++) {
		// //if
		// (Class.forName("eva2.server.modules.ModuleAdapter").isAssignableFrom((Class)al.get(i)))
