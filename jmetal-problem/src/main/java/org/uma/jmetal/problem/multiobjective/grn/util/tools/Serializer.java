...
public class Serializer {
	 ...
	 **/
	static public Object deepclone(final Serializable o) throws IOException,
			ClassNotFoundException {
		...
		Thread writer = new Thread() {
			public void run() {
				...
				try {
					...
					System.out.println("ERROR in Serialization1"
							+ e.getMessage());
				} finally {
					try {
						out.close();
					} catch (Exception e) {
						System.out.println("ERROR in Serialization2"
								+ e.getMessage());
					}
				}
			}
	...
	public static void storeString(String Filename, String s) {
		try {
			...
			System.out.println("ERROR writing string File " + Filename
					+ " String " + s);
		}
	}

	/**
	 * Deserialize the contents of File f containing a string and return the
	 * resulting string.
	 **/
	public static String loadString(String Filename) {
		...
		try {
			...
			// System.out.println("WARNING: Loading string File "+Filename+
			// " not possible !!");
		}
		return s;
	}
	...
	public static File storeObject(String Filename, Serializable s) {
		...
		try {
			...
			System.err.println("ERROR writing Object File " + Filename
					+ " String " + s);
	 ...
	 * return the resulting string. If the indicated file doesnt exist or an
	...
	public static Object loadObject(String Filename) {
		...
		if (f.exists()) {
			try {
				...
				System.err
						.println("WARNING: loading object File "
								+ Filename
