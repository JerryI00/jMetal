...
/**
 * This class is similar to 'java.util.ArrayList', except that it can only hold
 * and manage integer values.
 */
public class IntegerArrayList {
	...
	/**
	 * Clears the contents of this integer list and sets it back to it's initial
	 * state.
	 */
	public void clear() {
		integerArray = new int[initialSize];
	...
	/**
	 * Returns the integer value at the given position or 'min-value' if out of
	 * array bounds.
	 */
	public int get(int index) {
		if ((index < 0) || (index >= integerCount)) {
	 ...
	 * 
	 * @param exactLength
	 *            determines if the returned array should have the exactly right
	 *            length, or if longer arrays are allowed
	 */
	public int[] getIntegerArray(boolean exactLength) {
		// check if we can simply return our internal integer array
		if (!exactLength || (integerArray.length == integerCount)) {
			return integerArray;
		}
