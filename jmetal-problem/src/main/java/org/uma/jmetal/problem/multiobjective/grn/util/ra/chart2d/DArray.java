...
/**
 * This class stores an array of double values. It can be used as data stack for
 * an DPointSet object. For that use, it is important to tell the DPoitSet
 * object, when the data has been modified.
 */
public class DArray implements DIntDoubleMap {
	 ...
	 * @param initial_capacity
	 *            the initial capacity of the array
	 * @param capacity_multiplier
	 *            the capacity multiplier when the array overflows
	 */
	public DArray(int initial_capacity, double capacity_multiplier) {
		if (initial_capacity < 1)
			throw new IllegalArgumentException(
					"The initial capacity has to be at least 1");
		if (capacity_multiplier <= 1)
			throw new IllegalArgumentException(
					"The capacity multiplier has to be bigger than 1");
	...
	public double getMinImageValue() {
		if (size == 0)
			throw new IllegalArgumentException(
					"DArray is empty. No minimal value exists");
		return min;
	}
	...
	public double getMaxImageValue() {
		if (size == 0)
			throw new IllegalArgumentException(
					"DArray is empty. No maximal value exists");
		return max;
	}

	/**
	 * method checks if the given object is equal to this DArray object. It
	 * looks for differences in the stored image values
	 * 
	 * @param o
