...
public class EVAThread extends ArrayList implements Serializable {
	...
	public synchronized void doit(Object x, Method m, Object[] Para) {
		// System.out.println("calling doit "+m.getName()+ " of " +
		// x.hashCode());
...
final class XThread extends Thread implements Serializable {
	...
	public static XThread getXThread(Object x, Method m, Object[] Para) {
		while (m_instances > m_MAXinstances) {
			// System.out.println("waiting "+m_instances+ " on "+x.hashCode()+
			// " m "+m.getName()+" m_MAXinstances " +m_MAXinstances);
			try {
				Thread.sleep(5);
			} catch (Exception e) {
	...
	public void run() {
		if (m_method != null) {
			...
			try {
				// System.out.println("calling !!!!!!!!"+m_method.getName()+
				// " on " +m_x.hashCode()+" i = "+m_instances);
		...
		// System.out.println("finished !!!!!!!!"+m_method.getName()+ " on "
		// +m_x.hashCode()+ " i = "+m_instances);
	}
}
