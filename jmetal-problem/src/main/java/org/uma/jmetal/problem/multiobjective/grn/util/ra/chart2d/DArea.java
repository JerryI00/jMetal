...
import org.uma.jmetal.problem.multiobjective.grn.util.ra.print.PagePrinter;
...
public class DArea extends JComponent implements DParent, Printable {
	...
	public DRectangle getDRectangle() {
		...
		if (min_x != null)
			rect.x = Math.max(rect.x, getMinX());
		if (min_y != null)
			rect.y = Math.max(rect.y, getMinY());
		if (max_x != null)
			rect.width = Math.min(rect.width, getMaxX() - getMinX());
		if (max_y != null)
			rect.height = Math.min(rect.height, getMaxY() - getMinY());
		return rect;
	}
	...
	/**
	 * method sets the maximal rectangle whioch can be viewed with the DArea.
	 * This method can be used if the area is used with scale functions which
	 * are not invertible on all reals
	 * 
	 * @param x
	 *            the minmal x value
	 * @param y
	 ...
	 * 
	 * @param the
	 *            rect maximal rectangle of the DArea
	 * @deprecated see setMinX, setMinY, setMaxX, setMaxY
	 */
	public void setMaxRectangle(DRectangle rect) {
		if (!rect.contains(min_rect))
			throw new IllegalArgumentException(
					"Maximal rectangle does not contain minmal rectangle");
	...
	public void setMaxX(double max) {
		if (max < min_rect.x + min_rect.width)
			throw new IllegalArgumentException(
	...
	public void setMaxY(double may) {
		if (may < min_rect.y + min_rect.height)
			throw new IllegalArgumentException(
	...
	public void repaint(DRectangle r) {
		...
		if (r.isAll() || auto_focus)
			...
			Point p1 = measures.getPoint(r.x, r.y), p2 = measures.getPoint(r.x
	...
	private void paintGrid(DMeasures m) {
		...
		if (auto_grid) {
			...
			if (b instanceof ScaledBorder) {
				...
				grid.hor_dist = ScaledBorder.aBitBigger(grid.rectangle.width
						/ max_grid);
				grid.ver_dist = ScaledBorder.aBitBigger(grid.rectangle.height
						/ max_grid);
			}
		}
		grid.paint(m);
	...
	private void paintGrid(ScaledBorder sb, DMeasures m) {
		...
		if (m.x_scale == null && m.y_scale == null)
			...
			DRectangle rect = getDRectangle(), src_rect = m.getSourceOf(rect);

			int x = (int) (src_rect.x / grid.hor_dist), y = (int) (src_rect.y / grid.ver_dist);
			if (x * grid.hor_dist < src_rect.x)
				x++;
			if (y * grid.ver_dist < src_rect.y)
				y++;

			DPoint min = new DPoint(rect.x, rect.y), max = new DPoint(min.x
					+ rect.width, min.y + rect.height);
			...
			for (; (pos = x * grid.hor_dist) < src_rect.x + src_rect.width; x++) {
				...
				Point p1 = m.getPoint(pos, min.y), p2 = m.getPoint(pos, max.y);
			...
			for (; (pos = y * grid.ver_dist) < src_rect.y + src_rect.height; y++) {
				...
				Point p1 = m.getPoint(min.x, pos), p2 = m.getPoint(max.x, pos);
				g.drawLine(p1.x, p1.y, p2.x, p2.y);
			}
		}
	}
