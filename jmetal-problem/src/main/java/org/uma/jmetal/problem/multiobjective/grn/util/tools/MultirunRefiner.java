...
/**
 * MultirunRefiner Description: This is a small programm . Copyright: Copyright
 * (c) 2001 Company: University of Tuebingen, Computer Architecture
 * 
 * @author Felix Streichert
 * @version: $Revision: 10 $ $Date: 2006-01-18 11:02:22 +0100 (Wed, 18 Jan 2006)
 *           $ $Author: streiche $
 * @since JDK 1.3.0_02
 */
...
public class MultirunRefiner {
	...
	public void starter() {
		...
		this.m_ExitItem.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent evt) {
				System.exit(0);
			}
		});
		...
		this.m_Frame.getContentPane().add(this.myJButtonJPanel,
				BorderLayout.SOUTH);

		refineJButton = new JButton("Refine Multiruns");
		refineJButton.addMouseListener(new MouseAdapter() {
			public void mouseClicked(MouseEvent evt) {
				m_OutputText
						.setText(refineToText(refine(m_InputText.getText())));
			}
		});
	...
	/**
	 * This method lets you select a file that is then opened and displayed.
	 */
	...
	/**
	 * This method lets you select a destination file to save the current XML
	 * document
	 */
	...
	public static ArrayList<double[]> refine(String text) {
		...
		System.out.println(lineCnt + " lines parsed. " + numExp
				+ " experiments with " + result.size() + " events each.");
	...
	// tmp = this.parseStringForDouble(this.m_InputText.getText(begin,
	// end-begin));
	// if (tmp.length > 3) {
	// if (((int)(tmp[0])) == 1) numExp++;
	// if (result.size()-1 < ((int)(tmp[0]))) result.add(((int)(tmp[0])), new
	// double[3]);
	...
	// }
	// System.out.println(this.m_InputText.getLineCount() + " lines parsed. " +
	// numExp + " experiments with " + result.size() + " events each.");
	...
	public static double[] parseStringForDouble(String searchme) {
		...
		// because new Double(tmpString) does not regonize 2,3 as float(double)
		// i need to replace ','=>'.'
		...
		while (to < searchme.length()) {
			while ((to < searchme.length() - 1) && (searchme.charAt(to) != 9)
					&& (!Character.isSpaceChar(searchme.charAt(to)))
					&& (searchme.charAt(to) != '\n')) {
